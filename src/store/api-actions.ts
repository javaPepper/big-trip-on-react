...
import { setClickedAddNewButton, setClickedEdit, setClosed, setDataDestinations,
...
export const postNewPointAction = createAsyncThunk<void, Partial<Point>, {
  dispatch: AppDispatch;
  state: State;
  extra: AxiosInstance;
}>(
...
export const putEditPointAction = createAsyncThunk<void, Point, {
  ...
  async({base_price, date_from, date_to, destination, id, is_favorite, offers, type},
    {dispatch, extra: api, getState}) => {
    try {
      const {data} = await api.put<Point>(`${ApiRoutes.points}/${id as string}`,
        {base_price, date_from, date_to, destination, id, is_favorite, offers, type});
